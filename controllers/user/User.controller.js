                    ...
                    User.findOne({ name: name}, function (err, user){
                        if(user){
                            ...
                            userToCreate.save(function (err) {
                                ...
                                res.json({ error:false, message:"user inserted with success", user: userToCreate, token: token, code_http:200})
                            });
        ...
        User.findOne({
            ...
            if (!user) {
                ...
                if (user.password !== sha256(req.body.password + user.salt)) {
                    ...
                    let token = jwt.sign({"name": user.name, "password": user.password, "role": user.role, "email": user.email}, config.secret, {
                        expiresIn: "1d" // d h etc
                    });
    ...
    listUser:function(req,res){
        checkToken(req,function(data){ //data est la reponse JSON du test du token si success => token SINON error message classic
            if(data.error === true){
                ...
                //call mongoose
                User.find()   //TO DO  ajouter .populate('profiles')
                    .populate('profiles')
                    .exec()
                    .then(function(users){
                             res.json(users);
        ...
        /*
        Profile.find().populate('user').exec().then(function(data){
           res.send(data);
        });
        */
